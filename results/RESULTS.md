...
**Status:** All benchmarks recomputed after ADMM_SGL fix (rebuttal). piglasso_corr replaces plain piglasso in synthetic/dream5. Diffusion, SERGIO, and ci_coverage complete.
...
## TABLE 3 — piglasso_corr Scaling with n
...
piglasso_corr F1_opt at n=513 (0.791) nearly matches glasso grand mean (0.798), demonstrating convergence at large n. MCC at n=513 (0.750) exceeds all other methods.
...
| piglasso_corr | 9.6 | 30.9 | 470.5 |

desparsified is 20–600× faster than glasso. piglasso_corr is ~5× faster than plain piglasso at n513p164 (was 1700s; now 470s) because piglasso_corr uses Q=50 subsamples vs Q=200 for plain piglasso.
...
- All piglasso_corr, piglasso_string, piglasso (diffusion/SERGIO) results recomputed from scratch.
- Plain `piglasso` synthetic results (no prior, Q=200) not yet recomputed — pending resubmission.
...
- piglasso_corr MCC at n513p164 (0.750) is the highest MCC across all methods and configs.
...
### Pending
- Plain `piglasso` (no prior, Q=200) synthetic benchmark: stale results deleted, resubmission pending.
- n1026p328 config benchmarks: not included in grand means above.
